...
coin_img_two = pygame.image.load('coin_two.png')
scaled_coin = pygame.transform.scale(coin_img, (30,30))
scaled_coin_two = pygame.transform.scale(coin_img_two, (30,30))
...
def main():
    ...
    while running:
        ...
        SCREEN.blit(coin_score_txt, (WIDTH/1.8, 0))
        ...
        if pygame.sprite.spritecollideany(player, coins):
            ...
            number = random.randint(0, 1) #determine how much points will coin give
            if coin_obj.image == scaled_coin:
                coin_obj.coins += 1
            else:
                coin_obj.coins += 3

            if number == 0:
                coin_obj.image = scaled_coin #randomizing coins
            else:
                coin_obj.image = scaled_coin_two
            coin_obj.rect.center = (
                random.randint(coin_obj.rect.width, WIDTH - coin_obj.rect.width),
                0,
            )

        if coin_obj.coins == 15:
            enemy.speed += 5
